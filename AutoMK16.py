...
class EnhancedMultiRefAnnotator:
    ...
    This version ensures all feature detection, including fallbacks, is constrained to the reference ROI.
    """
    ...
    def _extract_superpoint_features(self, image, bbox=None):
        ...
        if bbox is not None and len(bbox) == 4:
            ...
            if w > 0 and h > 0 and x >= 0 and y >= 0 and (x + w) <= image.shape[1] and (y + h) <= image.shape[0]:
                ...
                crop_offset = (x, y)
                if self.debug:
                    print(f"   Debug: Cropping reference image to bbox {bbox} for SuperPoint extraction.")
            else:
                if self.debug:
    ...
    def _find_keypoints_with_traditional_method(self, ref_data, input_image, missing_keypoints, method='SIFT'):
        """
        FIXED: Use SIFT or ORB to find missing keypoints, constrained to the reference ROI.
        This is achieved by creating a mask from the reference bbox.
        """
        ...
        # --- FIX START: Create a mask from the reference bounding box ---
        ref_mask = None
        ...
        if bbox is not None and len(bbox) == 4:
            ...
            if w > 0 and h > 0:
                ...
                if self.debug:
                    print(f"   Debug: Created mask for {method} fallback from bbox {bbox}.")
        # --- FIX END ---
    ...
    def _visualize_enhanced_keypoints(self, image_path, matched_keypoints, all_robust_matches, debug_info=None):
        ...
        # Draw bounding box based on strategy
        bbox_for_drawing = None
        if self.bbox_strategy == 'total_matches' and all_robust_matches:
            if len(all_robust_matches) > 0:
                bbox_points = np.array(all_robust_matches)
                x_min, y_min = np.min(bbox_points, axis=0)
                x_max, y_max = np.max(bbox_points, axis=0)
                bbox_for_drawing = [int(x_min), int(y_min), int(x_max - x_min), int(y_max - y_min)]
    ...
    def create_coco_annotation(self, image_path, matched_keypoints, ref_id, all_robust_matches):
        """Create and add a COCO-formatted annotation."""
        ref_data = self.references.get(ref_id)
        ...
        annotation = {"id": self.annotation_id, "image_id": self.image_id, "category_id": 1, "keypoints": keypoints_coco, "num_keypoints": len(matched_keypoints), "bbox": bbox, "area": bbox[2] * bbox[3], "iscrowd": 0, "reference_used": ref_id}
    ...
    def process_images(self, input_folder, image_extensions=('.jpg', '.jpeg', '.png', '.bmp')):
        ...
        for i, image_path in enumerate(image_files, 1):
            ...
            try:
                ...
                if self.visualize: # No need to check best_ref_id again
...
def main():
    parser = argparse.ArgumentParser(description='Ultimate Auto Annotator with ROI-constrained fallbacks.')
    ...
    parser.add_argument('--bbox-strategy', type=str, default='keypoint', choices=['keypoint', 'total_matches'], help='Strategy for bounding box calculation.')
