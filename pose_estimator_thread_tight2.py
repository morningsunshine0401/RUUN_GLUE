...
class PoseEstimator:
    def __init__(self, opt, device):
        ...
        self.extractor = SuperPoint(max_num_keypoints=512).eval()
        self.matcher = LightGlue(features="superpoint").eval()

        if self.use_cuda:
            try:
                self.device = device  # Use the provided device
    ...
    def _convert_cv2_to_tensor(self, image):
        """
        Convert OpenCV BGR image to RGB tensor with comprehensive error handling
        """
        try:
            # Check if image is valid
            if image is None or not isinstance(image, np.ndarray):
                logger.error("Invalid image input in _convert_cv2_to_tensor")
            ...
            if np.max(image) > 255 or np.min(image) < 0:
                logger.warning("Image has values outside the 0-255 range, clipping")
            ...
            try:
                ...
                logger.error("Error in color conversion, using original image")
                image_rgb = image
            
            # Convert to float and normalize to [0, 1]
            image_float = image_rgb.astype(np.float32) / 255.0
            
            # Check normalization successful
            if np.max(image_float) > 1.0 or np.min(image_float) < 0.0:
                logger.warning("Normalization issue detected, fixing")
                image_float = np.clip(image_float, 0.0, 1.0)
            
            # Convert to tensor
            image_tensor = torch.from_numpy(image_float).float()
            
            # Permute dimensions from (H, W, C) to (C, H, W)
            image_tensor = image_tensor.permute(2, 0, 1).unsqueeze(0)
            
            try:
                # Try to send to device first without gradient
                with torch.no_grad():
                    return image_tensor.to(self.device, non_blocking=True)
            except RuntimeError as e:
                logger.warning(f"CUDA error when converting tensor to device: {e}")
                ...
                try:
                    ...
                    logger.error(f"Even CPU fallback failed: {cpu_error}")
                    return torch.zeros((1, 3, 224, 224), device='cpu')
        except Exception as e:
            logger.error(f"Unhandled exception in _convert_cv2_to_tensor: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return torch.zeros((1, 3, 224, 224), device='cpu')
    ...
    def process_frame(self, frame, frame_idx):
        ...
        - First frame: Use SuperPoint+LightGlue+PnP against anchor
        - Tracking mode: Frame-to-frame tracking with tightly-coupled MEKF
        - Reinitialization: Back to anchor matching when tracking fails
        """
        ...
        # CASE 1: First frame - always use PnP with anchor image
        if frame_idx == 1 or not self.kf_initialized:
            logger.info(f"Frame {frame_idx}: INITIALIZATION MODE - Using PnP with anchor")
            
            # Perform PnP estimation against anchor image
            pnp_pose_data, visualization, mkpts0, mkpts1, mpts3D, matches, known_mask = self.perform_pnp_estimation(
            ...
            # Check if pose is good enough for future tracking
            if pnp_pose_data['mean_reprojection_error'] < 4.0 and pnp_pose_data['num_inliers'] >= 8:
                # Good PnP pose - initialize MEKF and frame-to-frame tracking
                ...
                # Store current frame descriptors for future frame-to-frame matching
                match_indices = np.array(pnp_pose_data['inliers']).flatten()
                self.tracking_descriptors = frame_feats['descriptors'][0][:, match_indices].detach().cpu().numpy().T
                
                # Store the current frame features as previous frame for next iteration
                self.prev_frame_feats = frame_feats
                self.prev_frame_keypoints = frame_keypoints
        ...
        # CASE 2: Tracking mode - use frame-to-frame matching with tightly-coupled MEKF
        if self.kf_initialized and hasattr(self, 'prev_frame_feats'):
            logger.info(f"Frame {frame_idx}: TRACKING MODE - Using frame-to-frame matching")
            
            # Match features between previous frame and current frame
            with torch.no_grad():
                matches_dict = self.matcher({
                    'image0': self.prev_frame_feats,
                    'image1': frame_feats
                })
            
            # Remove batch dimension and move to CPU
            prev_feats, curr_feats, matches01 = [rbd(x) for x in [self.prev_frame_feats, frame_feats, matches_dict]]
            
            # Get keypoints and matches
            prev_kpts = self.prev_frame_keypoints
            matches = matches01["matches"].detach().cpu().numpy()
            confidence = matches01.get("scores", torch.ones(len(matches))).detach().cpu().numpy()
            
            if len(matches) < 6:
                logger.warning(f"Frame {frame_idx}: Too few matches for frame-to-frame tracking ({len(matches)})")
                # Fall through to reinitialization (case 3)
            else:
                # Predict next state using MEKF
                x_pred, P_pred = self.mekf.predict()
                
                # Track the 3D points from previous to current frame
                tracked_3D_points = []
                tracked_2D_points = []
                
                for i, pt3d in enumerate(self.tracking_3D_points):
                    # Find the keypoint index in previous frame that corresponds to this 3D point
                    if i < len(self.tracking_2D_points):
                        prev_2d = self.tracking_2D_points[i]
                        
                        # Find the closest previous frame keypoint
                        distances = np.linalg.norm(prev_kpts - prev_2d, axis=1)
                        closest_prev_idx = np.argmin(distances)
                        
                        # Check if it's a good match (close enough)
                        if distances[closest_prev_idx] < 5.0:
                            # Find matches from previous to current frame
                            matches_from_prev = matches[matches[:, 0] == closest_prev_idx]
                            
                            if len(matches_from_prev) > 0:
                                # Get the matching point in current frame
                                match_idx_curr = matches_from_prev[0, 1]
                                current_2d = frame_keypoints[match_idx_curr]
                                
                                tracked_3D_points.append(pt3d)
                                tracked_2D_points.append(current_2d)
                
                # Convert to numpy arrays
                if len(tracked_3D_points) > 0:
                    tracked_3D_points = np.array(tracked_3D_points)
                    tracked_2D_points = np.array(tracked_2D_points)
                    
                    # Need at least 3 points for tightly-coupled update
                    if len(tracked_3D_points) >= 4:
                        logger.info(f"Frame {frame_idx}: Successfully tracked {len(tracked_3D_points)} points")
                        
                        # Get camera parameters
                        K, distCoeffs = self._get_camera_intrinsics()
                        
                        # Update MEKF with tracked feature points
                        x_upd, P_upd = self.mekf.update_tightly_coupled(
                            tracked_2D_points, tracked_3D_points, K, distCoeffs
                        )
                        ...
                        visualization = self._visualize_tracking(
                            frame, tracked_2D_points, tracked_3D_points, x_upd, frame_idx
                        )
                        ...
                        pose_data = {
                            ...
                            'num_correspondences': len(tracked_3D_points),
                            'tracking_method': 'frame_to_frame_tracking'
                        }
                        
                        # Update tracking points for next frame
                        self.tracking_3D_points = tracked_3D_points
                        self.tracking_2D_points = tracked_2D_points
                        
                        # Store descriptors for tracked points
                        tracked_indices = []
                        for pt in tracked_2D_points:
                            distances = np.linalg.norm(frame_keypoints - pt, axis=1)
                            closest_idx = np.argmin(distances)
                            if distances[closest_idx] < 1.0:  # Very close match
                                tracked_indices.append(closest_idx)
                        
                        if len(tracked_indices) > 0:
                            self.tracking_descriptors = frame_feats['descriptors'][0][:, tracked_indices].detach().cpu().numpy().T
        ...
        # CASE 3: Reinitialization mode - fall back to anchor-based matching
        logger.info(f"Frame {frame_idx}: REINITIALIZATION MODE - Using anchor-based matching")
        ...
        if pnp_pose_data is None:
            # PnP failed but MEKF is initialized - use prediction
            if self.kf_initialized:
                logger.warning(f"Frame {frame_idx}: PnP reinitialization failed - using prediction only")
                
                # Get prediction from the filter
                x_pred, P_pred = self.mekf.predict()
        ...
        # PnP reinitialization succeeded
        if pnp_pose_data['mean_reprojection_error'] < 5.0 and pnp_pose_data['num_inliers'] >= 8:
            # Good PnP pose - reset frame-to-frame tracking
            logger.info(f"Frame {frame_idx}: Successful reinitialization with PnP")
            ...
            # If MEKF not initialized, create it
            if not self.kf_initialized:
                self.mekf = MultExtendedKalmanFilter(dt=1.0/30.0)
                self.kf_initialized = True
            
            # If MEKF is already initialized, we can use loosely-coupled update
            # to incorporate the PnP measurement
            if hasattr(self, 'mekf') and self.kf_initialized:
                ...
                # Add KF results to pose data
                pnp_pose_data['kf_translation_vector'] = position_upd.tolist()
                pnp_pose_data['kf_quaternion'] = quaternion_upd.tolist()
                pnp_pose_data['kf_rotation_matrix'] = R_upd.tolist()
                pnp_pose_data['tracking_method'] = 'reinitialized_with_kf'
            
            # Update tracking points for future frame-to-frame tracking
            inliers = np.array(pnp_pose_data['inliers'])
            self.tracking_3D_points = np.array(mpts3D)[inliers]
            self.tracking_2D_points = np.array(mkpts1)[inliers]
            ...
            for idx in match_indices:
                if idx < len(matches[:, 1][known_mask]):
                    valid_indices.append(matches[:, 1][known_mask][idx])
            
            if valid_indices:
                self.tracking_descriptors = frame_feats['descriptors'][0][:, valid_indices].detach().cpu().numpy().T
            ...
            # PnP succeeded but not good enough - use prediction if available
            logger.warning(f"Frame {frame_idx}: PnP reinitialization not good enough: " +
                    f"reprojection error = {pnp_pose_data['mean_reprojection_error']:.2f}")
            ...
            if self.kf_initialized:
                ...
                pose_data = {
                    ...
                    'kf_quaternion': quaternion_pred.tolist(),
                    'kf_rotation_matrix': R_pred.tolist(),
                    'object_translation_in_cam': pnp_pose_data['object_translation_in_cam'],
                    'object_rotation_in_cam': pnp_pose_data['object_rotation_in_cam'],
                    'pose_estimation_failed': False,
                    'tracking_method': 'prediction_with_poor_pnp',
                    'mean_reprojection_error': pnp_pose_data['mean_reprojection_error']
                }
                
                # Create simple visualization
                visualization = frame.copy()
                cv2.putText(visualization, "Poor PnP Quality - Using Prediction", 
                        (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                ...
                # No MEKF initialized - return poor PnP result
                return pnp_pose_data, visualization
    ...
    def perform_pnp_estimation(self, frame, frame_idx, frame_feats, frame_keypoints):
        ...
        return pose_data, visualization, mkpts0, mkpts1, mpts3D,matches,known_mask