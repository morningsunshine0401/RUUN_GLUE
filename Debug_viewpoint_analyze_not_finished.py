...
# At the top of your script, import cKDTree
...
def visualize_pose_and_matches(pose_data, image_dir, anchor_images_info,
    ...
    def update_visualization():
        ...
        # Remove the following lines since we'll compute these errors ourselves
        ...
        if num_inliers > 4 and 'rotation_matrix' in frame_data and 'translation_vector' in frame_data:
            ...
            if rotation_matrix.size == 9 and translation_vector.size == 3:
                ...
                # Reproject the 3D points associated with the inliers
                # We need to map the inlier indices to the corresponding 3D points
                # Assuming that mkpts0 corresponds to the anchor keypoints (2D)
                # and anchor_keypoints_2D corresponds to anchor_keypoints_3D

                # Find the indices of anchor keypoints that correspond to mkpts0
                # For this, we can match mkpts0 to anchor_keypoints_2D
                # Since mkpts0 are keypoints from the matching process, they may not directly correspond
                # to anchor_keypoints_2D indices
                # For simplicity, we can assume that mkpts0 indices correspond to anchor_keypoints_2D indices
                # if the matching was done directly on these keypoints

                anchor_kdtree = cKDTree(anchor_keypoints_2D)

                # For each mkpts0, find the nearest neighbor in anchor_keypoints_2D
                distances, indices = anchor_kdtree.query(mkpts0, distance_upper_bound=5)  # Adjust the threshold as needed

                # Filter out invalid matches (where distance is infinite)
                ...
                if len(indices) == 0:
                    print("No valid correspondences found between mkpts0 and anchor_keypoints_2D.")
                    return  # Cannot compute reprojection error without valid correspondences

                # Get corresponding 3D points
                matched_3D_points = anchor_keypoints_3D[indices]

                # Reproject the matched 3D points using the estimated pose
                reprojected_points, _ = cv2.projectPoints(matched_3D_points, rvec, translation_vector, K, distCoeffs)
                reprojected_points = reprojected_points.squeeze()

                # Ensure reprojected_points and mkpts1_valid have the same shape
                if reprojected_points.ndim == 1:
                    reprojected_points = reprojected_points[np.newaxis, :]
                if mkpts1_valid.ndim == 1:
                    mkpts1_valid = mkpts1_valid[np.newaxis, :]

                # Compute reprojection errors
                reproj_errors = np.linalg.norm(reprojected_points - mkpts1_valid, axis=1)
                mean_reproj_error = np.mean(reproj_errors)
                std_reproj_error = np.std(reproj_errors)

                # Plot the reprojected points in different colors for inliers and outliers
                for i, (x, y) in enumerate(reprojected_points):
                    ax_image.scatter(x + width1, y, s=50, c='green' if i in inliers else 'red', marker='x',
                                     label='Inliers Reprojection' if i == 0 and i in inliers else
                                           'Outliers Reprojection' if i == 0 and i not in inliers else "")
            ...
            # Compute rotation and translation errors if ground truth is available
            if gt_rotation_matrix is not None and gt_translation_vector is not None:
                # Compute rotation error
                R_err = rotation_matrix @ gt_rotation_matrix.T
                angle = np.arccos((np.trace(R_err) - 1) / 2)
                rotation_error_rad = np.abs(angle)  # Absolute value to avoid negative angles

                # Compute translation error
                translation_error = np.linalg.norm(translation_vector - gt_translation_vector.reshape(3, 1))
        ...
        if 'mean_reproj_error' in locals():
...
if __name__ == '__main__':
    ...
    pose_file = '/home/runbk0401/SuperGluePretrainedNetwork/pose_estimation_research_45.json'  # Replace with your actual path
