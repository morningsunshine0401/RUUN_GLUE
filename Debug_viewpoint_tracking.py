...
def visualize_pose_and_matches(pose_data, image_dir, anchor_images_info,
    ...
    camera_positions_tracking = []  # Initialize tracking camera positions list
    ...
    def update_visualization():
        ...
        # Get rotation and translation errors for PnP and Tracking
        rotation_error_pnp_rad = frame_data.get('rotation_error_pnp_rad', None)
        translation_error_pnp = frame_data.get('translation_error_pnp', None)
        rotation_error_tracking_rad = frame_data.get('rotation_error_tracking_rad', None)
        translation_error_tracking = frame_data.get('translation_error_tracking', None)
        ...
        # Reproject points using the estimated pose, only if inliers > 3
        if num_inliers > 3 and 'rotation_matrix' in frame_data and 'translation_vector' in frame_data:
            ...
            if rotation_matrix.size == 9 and translation_vector.size == 3:
                ...
                # Now process the camera positions
                ...
                # Compute estimated camera position from PnP
                ...
                ax_3d.scatter(camera_position_estimated_rotated[0], camera_position_estimated_rotated[1], camera_position_estimated_rotated[2],
                              c='r', marker='^', s=100, label='Current PnP Position')

                # Plot viewing direction for estimated pose
                forward_vector_estimated = rotation_matrix[:, 2]
                ...
                # Now process tracking pose
                if 'rotation_matrix_tracking' in frame_data and 'translation_vector_tracking' in frame_data:
                    rotation_matrix_tracking = np.array(frame_data.get('rotation_matrix_tracking', []))
                    translation_vector_tracking = np.array(frame_data.get('translation_vector_tracking', []))
                    if rotation_matrix_tracking.size == 9 and translation_vector_tracking.size == 3:
                        rotation_matrix_tracking = rotation_matrix_tracking.reshape(3, 3)
                        translation_vector_tracking = translation_vector_tracking.reshape(3, 1)
                        # Compute camera position from tracking
                        camera_position_tracking = -rotation_matrix_tracking.T @ translation_vector_tracking
                        # Rotate tracking camera position
                        camera_position_tracking_rotated = R_x_90 @ camera_position_tracking.flatten()
                        camera_positions_tracking.append(camera_position_tracking_rotated)
                        camera_positions_tracking_np = np.array(camera_positions_tracking)

                        # Plot the tracking camera positions
                        ax_3d.plot(camera_positions_tracking_np[:, 0], camera_positions_tracking_np[:, 1], camera_positions_tracking_np[:, 2],
                                   c='purple', marker='o', label='Tracking Camera Trajectory')

                        # Plot current tracking camera position
                        ax_3d.scatter(camera_positions_tracking_np[-1, 0], camera_positions_tracking_np[-1, 1], camera_positions_tracking_np[-1, 2],
                                      c='purple', marker='^', s=100, label='Current Tracking Position')
                    else:
                        print('Invalid rotation matrix or translation vector dimensions for tracking.')
                else:
                    print('Tracking pose not available in frame data.')
                ...
                if len(camera_positions_tracking) > 0:
                    all_x = np.concatenate((all_x, camera_positions_tracking_np[:, 0]))
                    all_y = np.concatenate((all_y, camera_positions_tracking_np[:, 1]))
                    all_z = np.concatenate((all_z, camera_positions_tracking_np[:, 2]))
                ...
                ax_3d.set_ylabel('Y')
                ax_3d.set_zlabel('Z')
                ax_3d.set_title('Estimated, Tracking, and Ground Truth Camera Poses')
        ...
        if rotation_error_pnp_rad is not None:
            rotation_error_pnp_deg = np.degrees(rotation_error_pnp_rad)
            print(f'PnP Rotation Error: {rotation_error_pnp_deg:.2f} degrees')
        if translation_error_pnp is not None:
            print(f'PnP Translation Error: {translation_error_pnp:.4f}')
        if rotation_error_tracking_rad is not None:
            rotation_error_tracking_deg = np.degrees(rotation_error_tracking_rad)
            print(f'Tracking Rotation Error: {rotation_error_tracking_deg:.2f} degrees')
        if translation_error_tracking is not None:
            print(f'Tracking Translation Error: {translation_error_tracking:.4f}')
...
if __name__ == '__main__':
    ...
    pose_file = '/home/runbk0401/SuperGluePretrainedNetwork/pose_estimation_research_56.json'  # Replace with your actual path
