...
class PoseEstimator:
    ...
    def _load_anchor_image(self):
        ...
        if self.opt.resize is not None:
            ...
            print('Will not resize anchor image.')
    ...
    def process_frame(self, frame, frame_idx):
        # Downscale input frame for SuperPoint processing
        input_frame_downscaled = cv2.resize(frame, (640, 360))
        frame_tensor = frame2tensor(input_frame_downscaled, self.device)

        # Extract features from the downscaled input frame
        with torch.no_grad():
        ...
        mkpts1 = frame_keypoints[matches[valid]]  # Input keypoints (downscaled resolution)
        mpts3D = self.matched_3D_keypoints[valid]
        mconf = confidence[valid]

        # Scale input keypoints to anchor resolution
        scale_x = self.anchor_image.shape[1] / input_frame_downscaled.shape[1]  # e.g., 1280 / 640
        scale_y = self.anchor_image.shape[0] / input_frame_downscaled.shape[0]  # e.g., 720 / 360
        mkpts1_scaled = mkpts1 * [scale_x, scale_y]

        total_matches = len(mkpts0)

        if total_matches >= 4:
            pose_data, visualization = self.estimate_pose(
                mkpts0, mkpts1_scaled, mpts3D, mconf, frame, frame_idx, frame_keypoints
            )
