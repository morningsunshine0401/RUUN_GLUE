...
class CharucoGroundTruthCalibrator:
    ...
    def solve_calibration(self):
        """Solve for tag-to-object transform using selected frames"""
        if len(self.final_calibration_frames) < 4:
            print("❌ Not enough calibration frames")
            return False
        
        print(f"\n🔧 Solving calibration with {len(self.final_calibration_frames)} frames...")
        
        # Prepare data for hand-eye calibration
        R_ct_list, t_ct_list = [], []  # Camera to Tag
        R_co_list, t_co_list = [], []  # Camera to Object
        
        for frame in self.final_calibration_frames:
            # ChArUco: Camera to Tag
            R_ct, _ = cv2.Rodrigues(frame.charuco_rvec)
            t_ct = frame.charuco_tvec.reshape(3, 1)
            R_ct_list.append(R_ct)
            t_ct_list.append(t_ct)
            
            # VAPE: Camera to Object
            # Scipy expects quaternion as (x, y, z, w), which is the format vape_quaternion is already in.
            R_co = R.from_quat(frame.vape_quaternion).as_matrix()
            t_co = frame.vape_position.reshape(3, 1)
            R_co_list.append(R_co)
            t_co_list.append(t_co)
        
        # Solve using OpenCV's hand-eye calibration
        try:
            R_to, t_to = cv2.calibrateHandEye(
                R_ct_list, t_ct_list,
                R_co_list, t_co_list,
                method=cv2.CALIB_HAND_EYE_TSAI
            )
            
            # Apply the known 25cm offset (tag is 25cm below CG in Y direction)
            # Adjust the translation to account for the pole
            t_to_corrected = t_to.reshape(3, 1)
            t_to_corrected[1] += 0.25  # Add 25cm in Y direction (up)
            ...
            self._validate_calibration(calibration_result, R_ct_list, t_ct_list, R_co_list, t_co_list)
