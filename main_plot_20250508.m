...
%jsonPath = '20250123_ICUAS_2_bright.json';
...
%% (13) 3D Trajectory Comparison (Raw vs KF)
figure('Name', '3D Trajectory Comparison (Raw vs KF)', 'NumberTitle', 'off', 'Position', [100, 100, 1000, 800]);
hold on; grid on; axis equal;
title('3D Trajectory: Raw vs KF', 'FontSize', 18);

% Plot raw trajectory if available
if any(~isnan(t_tgt_in_cam_raw(:)))
    plot3(t_tgt_in_cam_raw(selectedFrames,1), ...
          t_tgt_in_cam_raw(selectedFrames,2), ...
          t_tgt_in_cam_raw(selectedFrames,3), ...
          'b--', 'LineWidth', 2);
end

% Plot KF trajectory
if any(~isnan(t_tgt_in_cam_kf(:)))
    plot3(t_tgt_in_cam_kf(selectedFrames,1), ...
          t_tgt_in_cam_kf(selectedFrames,2), ...
          t_tgt_in_cam_kf(selectedFrames,3), ...
          'r-', 'LineWidth', 2);
end
...
legend({'Raw Trajectory', 'KF Trajectory'}, 'Location', 'best');
view(3);  % 3D view

print(gcf, fullfile(saveFolder, ...
    [jsonBase '_3DTrajectoryComparison.png']), '-dpng', '-r300');

%% (14) 3D Coordinate Frame Visualization (Orientation + Position)
figure('Name', '3D Pose Visualization (Raw vs KF)', 'NumberTitle', 'off', 'Position', [100, 100, 1000, 800]);
hold on; grid on; axis equal;
...
for idx = 1:step:length(selectedFrames)
    ...
    % Plot KF frames (in red-green-blue for x-y-z)
    if ~isempty(R_tgt_in_cam_kf{i}) && ~any(isnan(R_tgt_in_cam_kf{i}(:))) && ~any(isnan(t_tgt_in_cam_kf(i,:)))
        R = R_tgt_in_cam_kf{i};
        t = t_tgt_in_cam_kf(i,:)';
...
legend({'X axis (KF)', 'Y axis (KF)', 'Z axis (KF)'}, 'Location', 'best');
view(3);

print(gcf, fullfile(saveFolder, ...
    [jsonBase '_3DPoseFrames_KF.png']), '-dpng', '-r300');
