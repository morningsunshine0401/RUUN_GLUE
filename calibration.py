...
# # # Try detecting the asymmetric circles grid
# # found, centers = cv.findCirclesGrid(
# #     gray, dotGridSize, flags=cv.CALIB_CB_ASYMMETRIC_GRID
# # )

# # if found:
# #     print("Dot grid detected!")
# #     cv.drawChessboardCorners(image, dotGridSize, centers, found)
# #     cv.imshow("Detected Grid", image)
# #     cv.waitKey(0)
# #     cv.destroyAllWindows()
# # else:
# #     print("Dot grid not detected.")
...
# # Load video
# video_path = 'assets/Calibration/Opti/Opti_Phone.mp4'
...
# step = max(1, total_frames // 40)  # Ensure we don't process more than 30 frames

# frame_count = 0
# processed_frames = 0

# while processed_frames < 40:
#     ret, frame = cap.read()
#     if not ret:
#         break
...
#     # Detect dot grid
#     ret, centers = cv.findCirclesGrid(gray, dotGridSize, flags=cv.CALIB_CB_ASYMMETRIC_GRID)
#     if ret:
