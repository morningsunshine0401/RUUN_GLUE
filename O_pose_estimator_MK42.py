...
class PoseEstimator:
    def __init__(self, opt, device, kf_mode='auto'):
        ...
        self.extractor = SuperPoint(max_num_keypoints=64).eval().to(device)
        ...
        print(f"✅ SuperPoint+LightGlue initialized in {init_time:.1f}ms")
    ...
    def process_frame(self, frame, frame_idx, bbox=None, viewpoint=None):
        ...
        if bbox is not None:
            ...
            # === SuperPoint 특징 추출 ===
            pose_profiler.start_timer('superpoint_total')
            ...
            superpoint_time = pose_profiler.end_timer('superpoint_total')
            ...
            if reprojection_error < 4.0 and num_inliers >= 5:
                ...
                # 성능 통계 (주기적으로만)
                if frame_idx % 30 == 0:
                    print(f"📊 Frame {frame_idx}: SP {superpoint_time:.1f}ms, PnP {pnp_time:.1f}ms, "
    ...
    def enhance_pose_initialization(self, initial_pose, mkpts0, mkpts1, mpts3D, frame):
        ...
        for i, model_pt in enumerate(all_3d_points):
            ...
            if min_dist < 3.0: #5:  # Threshold in pixels
    ...
    def perform_pnp_estimation(self, frame, frame_idx, frame_feats, frame_keypoints):
        ...
        success, rvec_o, tvec_o, inliers = cv2.solvePnPRansac(
            ...
            iterationsCount=1000,#2500,#1500,#500,
            flags=cv2.SOLVEPNP_EPNP
        )

        if not success or inliers is None or len(inliers) < 5:#6:#4:#6:
            print("INLIERS ARE :\n",len(inliers))
